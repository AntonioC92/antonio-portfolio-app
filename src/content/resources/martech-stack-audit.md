...
Most marketing teams cannot list every tool in their stack without opening three different browser tabs and asking finance for the credit card statement. That is the real starting point for a martech audit: not a strategy problem, a visibility problem. We have run this audit with teams ranging from three-person startups to established B2B companies, and the finding is almost always the same. Nobody owns the full picture, and the stack has grown by accumulation rather than design.
...
- **Morning (inventory):** list every tool with an owner, a login, and a monthly or annual cost. Pull this from finance records, not memory.
...
- **Afternoon (integration map):** trace how data actually flows between tools, not how it was supposed to flow when it was set up.
- **End of day (decision list):** sort every tool into keep, consolidate, or cut, with a one-line reason for each.

The output is not a polished deck. It is a working document your team can act on immediately.

## Why this needs a day, not a workshop series
...
A one-day audit works because it forces triage instead of analysis paralysis. You are not trying to build the perfect long-term architecture in eight hours. You are trying to answer three questions clearly enough to make near-term decisions: what do we have, what is it doing, and what should change.

If you want the longer-term picture once the audit is done, [our framework for what a modern marketing system actually looks like](/insights/modern-marketing-system-2025) covers how to design the target state. This audit is the input to that work, not a replacement for it.
...
3. **Purpose** — one sentence, written by the owner, not assumed by you.
4. **Category** — which function it serves: CRM, email, analytics, ads, content, forms, and so on.

Do not skip tools that seem obviously fine. The point of the inventory is completeness, not judgment. Judgment comes in the afternoon.

By late morning you should have a flat list of every tool in the stack, typically somewhere between 15 and 40 for a small-to-mid-size marketing team. That number alone is often the first useful finding, since most teams underestimate it by half.

## Midday: check actual usage, not assumed usage
...
- **Last login date** across all users, not just the admin.
