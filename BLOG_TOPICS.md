...
Topics for `src/content/resources/`. Claude picks the first `[ ]` item when generating a new post.

Mark `[x]` when written and committed. Add new topics at the bottom of each section.
...
- [x] **GA4 event naming conventions: a practical guide** — `ga4-event-naming` — Target: "ga4 event naming" — The naming decisions that make GA4 actually usable across teams. (Already published; checkbox was stale.)
...
- [x] **Measuring what an AI Overviews citation is actually worth** — `measuring-ai-overviews-citation-value` — Target: "measure ai overviews citation value" — Added 2026-09-24 (off-queue, trend-driven): CTR on AI Overview queries fell as much as 58% in 2026 per Ahrefs and Pew Research, but Seer Interactive's April 2026 study of 2.43B impressions found cited brands still earn roughly twice the clicks per impression of uncited ones. A framework for tracking citation depth and mention rate instead of relying on click data alone, since the standard queue (marketing-data-source-of-truth, ai-time-savings-marketing-team, questions-before-hiring-fractional-cmo) is already covered by open unmerged branches.
- [x] **How to track which sources AI cites for your competitors** — `ai-citation-competitor-tracking` — Target: "track competitor ai citations" — Added 2026-09-26 (off-queue, trend-driven): the standard queue (marketing-data-source-of-truth, ai-time-savings-marketing-team, questions-before-hiring-fractional-cmo) is still fully covered by open unmerged branches. Search Console showed real, currently unranked query intent for this exact angle ("how do i find out which sources ai cites for my competitors," 8 impressions, position 60), and a monitoring tool ecosystem (Ahrefs Brand Radar, Otterly.ai, and others) has grown up around AI share-of-voice tracking in 2026. A method for widening a brand's own AI visibility prompt set to log competitor citations too, and turning the gap into a content or outreach plan.

---

## Completed

<!-- Move entries here (with [x]) once published -->
